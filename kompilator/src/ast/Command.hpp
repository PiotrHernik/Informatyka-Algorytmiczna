...
struct Command : Node
{
    virtual std::vector<std::string> executeCommand(SymbolTable&, std::vector<std::shared_ptr<Procedure>>&, bool = false) = 0;
...
struct CommandAssign : Command
{
    ...
    std::vector<std::string> executeCommand(SymbolTable&, std::vector<std::shared_ptr<Procedure>>&, bool = false) override;
